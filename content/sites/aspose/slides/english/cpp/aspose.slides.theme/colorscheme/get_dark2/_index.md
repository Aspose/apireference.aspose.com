---
title: get_Dark2()
second_title: Aspose.Slides for C++ API Reference
description: Second dark color in the scheme. Read-only IColorFormat.
type: docs
weight: 27
url: /cpp/aspose.slides.theme/colorscheme/get_dark2/
---
## ColorScheme::get_Dark2() method


Second dark color in the scheme. Read-only [IColorFormat](../../../aspose.slides/icolorformat/).

```cpp
System::SharedPtr<IColorFormat> Aspose::Slides::Theme::ColorScheme::get_Dark2() override
```

## See Also

* Typedef [SharedPtr](../../system/sharedptr/)
* Class [IColorFormat](../../aspose.slides/icolorformat/)
* Class [ColorScheme](./)
* Namespace [Aspose::Slides::Theme](../)
* Library [Aspose.Slides](../../)