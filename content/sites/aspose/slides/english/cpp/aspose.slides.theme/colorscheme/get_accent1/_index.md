---
title: get_Accent1
second_title: Aspose.Slides for C++ API Reference
description: First accent color in the scheme. Read-only IColorFormat.
type: docs
weight: 53
url: /cpp/aspose.slides.theme/colorscheme/get_accent1/
---
## ColorScheme::get_Accent1 method


First accent color in the scheme. Read-only [IColorFormat](../../../aspose.slides/icolorformat/).

```cpp
System::SharedPtr<IColorFormat> Aspose::Slides::Theme::ColorScheme::get_Accent1() override
```

## See Also

* Typedef [SharedPtr](../../../system/sharedptr/)
* Class [IColorFormat](../../../aspose.slides/icolorformat/)
* Class [ColorScheme](../)
* Namespace [Aspose::Slides::Theme](../../)
* Library [Aspose.Slides](../../../)
