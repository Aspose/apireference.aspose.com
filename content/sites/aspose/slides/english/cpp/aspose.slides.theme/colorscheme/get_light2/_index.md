---
title: get_Light2
second_title: Aspose.Slides for C++ API Reference
description: Second light color in the scheme. Read-only IColorFormat.
type: docs
weight: 40
url: /cpp/aspose.slides.theme/colorscheme/get_light2/
---
## ColorScheme::get_Light2 method


Second light color in the scheme. Read-only [IColorFormat](../../../aspose.slides/icolorformat/).

```cpp
System::SharedPtr<IColorFormat> Aspose::Slides::Theme::ColorScheme::get_Light2() override
```

## See Also

* Typedef [SharedPtr](../../../system/sharedptr/)
* Class [IColorFormat](../../../aspose.slides/icolorformat/)
* Class [ColorScheme](../)
* Namespace [Aspose::Slides::Theme](../../)
* Library [Aspose.Slides](../../../)
