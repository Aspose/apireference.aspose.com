---
title: get_Accent4
second_title: Aspose.Slides for C++ API Reference
description: Fourth accent color in the scheme. Read-only IColorFormat.
type: docs
weight: 92
url: /cpp/aspose.slides.theme/colorscheme/get_accent4/
---
## ColorScheme::get_Accent4 method


Fourth accent color in the scheme. Read-only [IColorFormat](../../../aspose.slides/icolorformat/).

```cpp
System::SharedPtr<IColorFormat> Aspose::Slides::Theme::ColorScheme::get_Accent4() override
```

## See Also

* Typedef [SharedPtr](../../../system/sharedptr/)
* Class [IColorFormat](../../../aspose.slides/icolorformat/)
* Class [ColorScheme](../)
* Namespace [Aspose::Slides::Theme](../../)
* Library [Aspose.Slides](../../../)
