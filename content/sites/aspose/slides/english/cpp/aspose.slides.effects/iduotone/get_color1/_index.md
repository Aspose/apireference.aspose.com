---
title: get_Color1
second_title: Aspose.Slides for C++ API Reference
description: Returns target color format for dark pixels. Read-only IColorFormat.
type: docs
weight: 1
url: /cpp/aspose.slides.effects/iduotone/get_color1/
---
## IDuotone::get_Color1 method


Returns target color format for dark pixels. Read-only [IColorFormat](../../../aspose.slides/icolorformat/).

```cpp
virtual System::SharedPtr<IColorFormat> Aspose::Slides::Effects::IDuotone::get_Color1()=0
```

## See Also

* Typedef [SharedPtr](../../../system/sharedptr/)
* Class [IColorFormat](../../../aspose.slides/icolorformat/)
* Class [IDuotone](../)
* Namespace [Aspose::Slides::Effects](../../)
* Library [Aspose.Slides](../../../)
