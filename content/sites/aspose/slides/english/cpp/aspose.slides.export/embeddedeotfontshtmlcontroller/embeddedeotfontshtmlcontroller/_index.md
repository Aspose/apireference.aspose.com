---
title: EmbeddedEotFontsHtmlController()
second_title: Aspose.Slides for C++ API Reference
description: Creates new instance.
type: docs
weight: 1
url: /cpp/aspose.slides.export/embeddedeotfontshtmlcontroller/embeddedeotfontshtmlcontroller/
---
## EmbeddedEotFontsHtmlController::EmbeddedEotFontsHtmlController() constructor


Creates new instance.

```cpp
Aspose::Slides::Export::EmbeddedEotFontsHtmlController::EmbeddedEotFontsHtmlController()
```

## EmbeddedEotFontsHtmlController::EmbeddedEotFontsHtmlController(System::SharedPtr\<IHtmlFormattingController\>) constructor


Creates new instance.

```cpp
Aspose::Slides::Export::EmbeddedEotFontsHtmlController::EmbeddedEotFontsHtmlController(System::SharedPtr<IHtmlFormattingController> controller)
```


### Arguments

| Parameter | Type | Description |
| --- | --- | --- |
| controller | [System::SharedPtr](../../../system/sharedptr/)\<[IHtmlFormattingController](../../ihtmlformattingcontroller/)\> | HTML formatting controller. |

## See Also

* Typedef [SharedPtr](../../system/sharedptr/)
* Class [EmbeddedEotFontsHtmlController](./)
* Class [IHtmlFormattingController](../ihtmlformattingcontroller/)
* Namespace [Aspose::Slides::Export](../)
* Library [Aspose.Slides](../../)