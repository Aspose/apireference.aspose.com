---
title: RangeItemHeaderValue()
second_title: Aspose.Slides for C++ API Reference
description: Constructs a new instance.
type: docs
weight: 27
url: /cpp/system.net.http.headers/rangeitemheadervalue/rangeitemheadervalue/
---
## RangeItemHeaderValue::RangeItemHeaderValue(Nullable\<int64_t\>, Nullable\<int64_t\>) constructor


Constructs a new instance.

```cpp
System::Net::Http::Headers::RangeItemHeaderValue::RangeItemHeaderValue(Nullable<int64_t> from, Nullable<int64_t> to)
```


### Arguments

| Parameter | Type | Description |
| --- | --- | --- |
| from | [Nullable](../../../system/nullable/)\<**int64_t**\> | The position at which data sending must start. |
| to | [Nullable](../../../system/nullable/)\<**int64_t**\> | The position at which data sending must stop. |

## See Also

* Class [Nullable](../../system/nullable/)
* Class [RangeItemHeaderValue](./)
* Namespace [System::Net::Http::Headers](../)
* Library [Aspose.Slides](../../)