---
title: set_ShapeTypeLocked()
second_title: Aspose.Slides for C++ API Reference
description: Determines whether a changing of a shape type is forbidden. Write bool.
type: docs
weight: 248
url: /cpp/aspose.slides/connectorlock/set_shapetypelocked/
---
## ConnectorLock::set_ShapeTypeLocked(bool) method


Determines whether a changing of a shape type is forbidden. Write **bool**.

```cpp
void Aspose::Slides::ConnectorLock::set_ShapeTypeLocked(bool value) override
```

## See Also

* Class [ConnectorLock](./)
* Namespace [Aspose::Slides](../)
* Library [Aspose.Slides](../../)