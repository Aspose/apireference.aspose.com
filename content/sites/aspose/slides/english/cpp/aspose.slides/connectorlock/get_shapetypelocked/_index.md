---
title: get_ShapeTypeLocked
second_title: Aspose.Slides for C++ API Reference
description: Determines whether a changing of a shape type is forbidden. Read bool.
type: docs
weight: 235
url: /cpp/aspose.slides/connectorlock/get_shapetypelocked/
---
## ConnectorLock::get_ShapeTypeLocked method


Determines whether a changing of a shape type is forbidden. Read **bool**.

```cpp
bool Aspose::Slides::ConnectorLock::get_ShapeTypeLocked() override
```

## See Also

* Class [ConnectorLock](../)
* Namespace [Aspose::Slides](../../)
* Library [Aspose.Slides](../../../)
