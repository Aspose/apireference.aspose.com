---
title: get_HyperlinkClick()
second_title: Aspose.Slides for C++ API Reference
description: Returns the hyperlink defined for mouse click. Read IHyperlink.
type: docs
weight: 53
url: /cpp/aspose.slides/portionformat/get_hyperlinkclick/
---
## PortionFormat::get_HyperlinkClick() method


Returns the hyperlink defined for mouse click. Read [IHyperlink](../../ihyperlink/).

```cpp
System::SharedPtr<IHyperlink> Aspose::Slides::PortionFormat::get_HyperlinkClick() override
```

## See Also

* Typedef [SharedPtr](../../system/sharedptr/)
* Class [IHyperlink](../ihyperlink/)
* Class [PortionFormat](./)
* Namespace [Aspose::Slides](../)
* Library [Aspose.Slides](../../)