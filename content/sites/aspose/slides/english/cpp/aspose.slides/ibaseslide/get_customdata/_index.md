---
title: get_CustomData()
second_title: Aspose.Slides for C++ API Reference
description: Returns the slide's custom data. Read-only ICustomData.
type: docs
weight: 66
url: /cpp/aspose.slides/ibaseslide/get_customdata/
---
## IBaseSlide::get_CustomData() method


Returns the slide's custom data. Read-only [ICustomData](../../icustomdata/).

```cpp
virtual System::SharedPtr<ICustomData> Aspose::Slides::IBaseSlide::get_CustomData()=0
```

## See Also

* Typedef [SharedPtr](../../system/sharedptr/)
* Class [ICustomData](../icustomdata/)
* Class [IBaseSlide](./)
* Namespace [Aspose::Slides](../)
* Library [Aspose.Slides](../../)