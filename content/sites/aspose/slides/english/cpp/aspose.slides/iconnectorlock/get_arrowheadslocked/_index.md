---
title: get_ArrowheadsLocked
second_title: Aspose.Slides for C++ API Reference
description: Determines whether a changing arrowheads is forbidden. Read bool.
type: docs
weight: 209
url: /cpp/aspose.slides/iconnectorlock/get_arrowheadslocked/
---
## IConnectorLock::get_ArrowheadsLocked method


Determines whether a changing arrowheads is forbidden. Read **bool**.

```cpp
virtual bool Aspose::Slides::IConnectorLock::get_ArrowheadsLocked()=0
```

## See Also

* Class [IConnectorLock](../)
* Namespace [Aspose::Slides](../../)
* Library [Aspose.Slides](../../../)
