---
title: set_ShapeTypeLocked()
second_title: Aspose.Slides for C++ API Reference
description: Determines whether a changing of a shape type is forbidden. Write bool.
type: docs
weight: 248
url: /cpp/aspose.slides/ipictureframelock/set_shapetypelocked/
---
## IPictureFrameLock::set_ShapeTypeLocked(bool) method


Determines whether a changing of a shape type is forbidden. Write **bool**.

```cpp
virtual void Aspose::Slides::IPictureFrameLock::set_ShapeTypeLocked(bool value)=0
```

## See Also

* Class [IPictureFrameLock](./)
* Namespace [Aspose::Slides](../)
* Library [Aspose.Slides](../../)