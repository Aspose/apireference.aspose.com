---
title: get_IsEncrypted()
second_title: Aspose.Slides for C++ API Reference
description: Gets True if binded presentation is encrypted, otherwise False. Read-only bool.
type: docs
weight: 1
url: /cpp/aspose.slides/ipresentationinfo/get_isencrypted/
---
## IPresentationInfo::get_IsEncrypted() method


Gets True if binded presentation is encrypted, otherwise False. Read-only **bool**.

```cpp
virtual bool Aspose::Slides::IPresentationInfo::get_IsEncrypted()=0
```

## See Also

* Class [IPresentationInfo](./)
* Namespace [Aspose::Slides](../)
* Library [Aspose.Slides](../../)