---
title: get_SelectLocked()
second_title: Aspose.Slides for C++ API Reference
description: Determines whether a selecting this shape is forbidden. Read bool.
type: docs
weight: 27
url: /cpp/aspose.slides/pictureframelock/get_selectlocked/
---
## PictureFrameLock::get_SelectLocked() method


Determines whether a selecting this shape is forbidden. Read **bool**.

```cpp
bool Aspose::Slides::PictureFrameLock::get_SelectLocked() override
```

## See Also

* Class [PictureFrameLock](./)
* Namespace [Aspose::Slides](../)
* Library [Aspose.Slides](../../)