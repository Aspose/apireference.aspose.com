---
title: get_PositionLocked
second_title: Aspose.Slides for C++ API Reference
description: Determines whether a moving this shape is forbidden. Read bool.
type: docs
weight: 105
url: /cpp/aspose.slides/pictureframelock/get_positionlocked/
---
## PictureFrameLock::get_PositionLocked method


Determines whether a moving this shape is forbidden. Read **bool**.

```cpp
bool Aspose::Slides::PictureFrameLock::get_PositionLocked() override
```

## See Also

* Class [PictureFrameLock](../)
* Namespace [Aspose::Slides](../../)
* Library [Aspose.Slides](../../../)
