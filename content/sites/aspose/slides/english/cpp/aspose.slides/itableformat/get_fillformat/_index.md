---
title: get_FillFormat
second_title: Aspose.Slides for C++ API Reference
description: Returns a table fill properties object. Read-only IFillFormat.
type: docs
weight: 1
url: /cpp/aspose.slides/itableformat/get_fillformat/
---
## ITableFormat::get_FillFormat method


Returns a table fill properties object. Read-only [IFillFormat](../../ifillformat/).

```cpp
virtual System::SharedPtr<IFillFormat> Aspose::Slides::ITableFormat::get_FillFormat()=0
```

## See Also

* Typedef [SharedPtr](../../../system/sharedptr/)
* Class [IFillFormat](../../ifillformat/)
* Class [ITableFormat](../)
* Namespace [Aspose::Slides](../../)
* Library [Aspose.Slides](../../../)
