---
title: get_TitleStyle()
second_title: Aspose.Slides for C++ API Reference
description: Returns the style of a title text. Read-only ITextStyle.
type: docs
weight: 14
url: /cpp/aspose.slides/imasterslide/get_titlestyle/
---
## IMasterSlide::get_TitleStyle() method


Returns the style of a title text. Read-only [ITextStyle](../../itextstyle/).

```cpp
virtual System::SharedPtr<ITextStyle> Aspose::Slides::IMasterSlide::get_TitleStyle()=0
```

## See Also

* Typedef [SharedPtr](../../system/sharedptr/)
* Class [ITextStyle](../itextstyle/)
* Class [IMasterSlide](./)
* Namespace [Aspose::Slides](../)
* Library [Aspose.Slides](../../)