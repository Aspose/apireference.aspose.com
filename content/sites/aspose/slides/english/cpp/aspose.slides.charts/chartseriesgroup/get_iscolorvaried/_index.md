---
title: get_IsColorVaried
second_title: Aspose.Slides for C++ API Reference
description: Specifies that each data marker in the series has a different color. Read bool.
type: docs
weight: 287
url: /cpp/aspose.slides.charts/chartseriesgroup/get_iscolorvaried/
---
## ChartSeriesGroup::get_IsColorVaried method


Specifies that each data marker in the series has a different color. Read **bool**.

```cpp
bool Aspose::Slides::Charts::ChartSeriesGroup::get_IsColorVaried() override
```

## See Also

* Class [ChartSeriesGroup](../)
* Namespace [Aspose::Slides::Charts](../../)
* Library [Aspose.Slides](../../../)
