---
title: get_HasSeriesLines
second_title: Aspose.Slides for C++ API Reference
description: True if chart has series lines. Applied to stacked bar and OfPie charts. Read bool.
type: docs
weight: 313
url: /cpp/aspose.slides.charts/chartseriesgroup/get_hasserieslines/
---
## ChartSeriesGroup::get_HasSeriesLines method


True if chart has series lines. Applied to stacked bar and OfPie charts. Read **bool**.

```cpp
bool Aspose::Slides::Charts::ChartSeriesGroup::get_HasSeriesLines() override
```

## See Also

* Class [ChartSeriesGroup](../)
* Namespace [Aspose::Slides::Charts](../../)
* Library [Aspose.Slides](../../../)
