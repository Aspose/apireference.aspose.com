---
title: get_Effect
second_title: Aspose.Slides for C++ API Reference
description: Returns effects used for a chart line. Read-only IEffectFormat.
type: docs
weight: 14
url: /cpp/aspose.slides.charts/ichartlinesformat/get_effect/
---
## IChartLinesFormat::get_Effect method


Returns effects used for a chart line. Read-only [IEffectFormat](../../../aspose.slides/ieffectformat/).

```cpp
virtual System::SharedPtr<IEffectFormat> Aspose::Slides::Charts::IChartLinesFormat::get_Effect()=0
```

## See Also

* Typedef [SharedPtr](../../../system/sharedptr/)
* Class [IEffectFormat](../../../aspose.slides/ieffectformat/)
* Class [IChartLinesFormat](../)
* Namespace [Aspose::Slides::Charts](../../)
* Library [Aspose.Slides](../../../)
