---
title: get_IsBubble3D
second_title: Aspose.Slides for C++ API Reference
description: Specifies that the bubbles have a 3-D effect applied to them. Read bool.
type: docs
weight: 105
url: /cpp/aspose.slides.charts/chartdatapoint/get_isbubble3d/
---
## ChartDataPoint::get_IsBubble3D method


Specifies that the bubbles have a 3-D effect applied to them. Read **bool**.

```cpp
bool Aspose::Slides::Charts::ChartDataPoint::get_IsBubble3D() override
```

## See Also

* Class [ChartDataPoint](../)
* Namespace [Aspose::Slides::Charts](../../)
* Library [Aspose.Slides](../../../)
