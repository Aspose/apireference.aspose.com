---
title: set_Item
second_title: Aspose.Sildes for Node.js via Java API Reference
description: 
type: docs

url: /nodejs-java/aspose.slides/mathblock/set_item/
---

## set_Item(int index, [MathFraction](../../mathfraction) value)  function

 Gets or sets IMathElement at the specified index.
 

### Parameters

| Name | Type | Description |
| --- | --- | --- |
| value | int | The mathematical element. |
| index | [MathFraction](../../mathfraction) | The zero-based index of the item |


---


## set_Item(int index, [MathMatrix](../../mathmatrix) value)  function

 Gets or sets IMathElement at the specified index.
 

### Parameters

| Name | Type | Description |
| --- | --- | --- |
| value | int | The mathematical element. |
| index | [MathMatrix](../../mathmatrix) | The zero-based index of the item |


---


## set_Item(int index, [MathSuperscriptElement](../../mathsuperscriptelement) value)  function

 Gets or sets IMathElement at the specified index.
 

### Parameters

| Name | Type | Description |
| --- | --- | --- |
| value | int | The mathematical element. |
| index | [MathSuperscriptElement](../../mathsuperscriptelement) | The zero-based index of the item |


---


## set_Item(int index, [MathAccent](../../mathaccent) value)  function

 Gets or sets IMathElement at the specified index.
 

### Parameters

| Name | Type | Description |
| --- | --- | --- |
| value | int | The mathematical element. |
| index | [MathAccent](../../mathaccent) | The zero-based index of the item |


---


## set_Item(int index, [MathLeftSubSuperscriptElement](../../mathleftsubsuperscriptelement) value)  function

 Gets or sets IMathElement at the specified index.
 

### Parameters

| Name | Type | Description |
| --- | --- | --- |
| value | int | The mathematical element. |
| index | [MathLeftSubSuperscriptElement](../../mathleftsubsuperscriptelement) | The zero-based index of the item |


---


## set_Item(int index, [MathBar](../../mathbar) value)  function

 Gets or sets IMathElement at the specified index.
 

### Parameters

| Name | Type | Description |
| --- | --- | --- |
| value | int | The mathematical element. |
| index | [MathBar](../../mathbar) | The zero-based index of the item |


---


## set_Item(int index, [MathSubscriptElement](../../mathsubscriptelement) value)  function

 Gets or sets IMathElement at the specified index.
 

### Parameters

| Name | Type | Description |
| --- | --- | --- |
| value | int | The mathematical element. |
| index | [MathSubscriptElement](../../mathsubscriptelement) | The zero-based index of the item |


---


## set_Item(int index, [MathFunction](../../mathfunction) value)  function

 Gets or sets IMathElement at the specified index.
 

### Parameters

| Name | Type | Description |
| --- | --- | --- |
| value | int | The mathematical element. |
| index | [MathFunction](../../mathfunction) | The zero-based index of the item |


---


## set_Item(int index, [MathematicalText](../../mathematicaltext) value)  function

 Gets or sets IMathElement at the specified index.
 

### Parameters

| Name | Type | Description |
| --- | --- | --- |
| value | int | The mathematical element. |
| index | [MathematicalText](../../mathematicaltext) | The zero-based index of the item |


---


## set_Item(int index, [MathRightSubSuperscriptElement](../../mathrightsubsuperscriptelement) value)  function

 Gets or sets IMathElement at the specified index.
 

### Parameters

| Name | Type | Description |
| --- | --- | --- |
| value | int | The mathematical element. |
| index | [MathRightSubSuperscriptElement](../../mathrightsubsuperscriptelement) | The zero-based index of the item |


---


## set_Item(int index, [MathDelimiter](../../mathdelimiter) value)  function

 Gets or sets IMathElement at the specified index.
 

### Parameters

| Name | Type | Description |
| --- | --- | --- |
| value | int | The mathematical element. |
| index | [MathDelimiter](../../mathdelimiter) | The zero-based index of the item |


---


## set_Item(int index, [MathBox](../../mathbox) value)  function

 Gets or sets IMathElement at the specified index.
 

### Parameters

| Name | Type | Description |
| --- | --- | --- |
| value | int | The mathematical element. |
| index | [MathBox](../../mathbox) | The zero-based index of the item |


---


## set_Item(int index, [MathBlock](../../mathblock) value)  function

 Gets or sets IMathElement at the specified index.
 

### Parameters

| Name | Type | Description |
| --- | --- | --- |
| value | int | The mathematical element. |
| index | [MathBlock](../../mathblock) | The zero-based index of the item |


---


## set_Item(int index, [MathLimit](../../mathlimit) value)  function

 Gets or sets IMathElement at the specified index.
 

### Parameters

| Name | Type | Description |
| --- | --- | --- |
| value | int | The mathematical element. |
| index | [MathLimit](../../mathlimit) | The zero-based index of the item |


---


## set_Item(int index, [MathArray](../../matharray) value)  function

 Gets or sets IMathElement at the specified index.
 

### Parameters

| Name | Type | Description |
| --- | --- | --- |
| value | int | The mathematical element. |
| index | [MathArray](../../matharray) | The zero-based index of the item |


---


## set_Item(int index, [MathGroupingCharacter](../../mathgroupingcharacter) value)  function

 Gets or sets IMathElement at the specified index.
 

### Parameters

| Name | Type | Description |
| --- | --- | --- |
| value | int | The mathematical element. |
| index | [MathGroupingCharacter](../../mathgroupingcharacter) | The zero-based index of the item |


---


## set_Item(int index, [MathNaryOperator](../../mathnaryoperator) value)  function

 Gets or sets IMathElement at the specified index.
 

### Parameters

| Name | Type | Description |
| --- | --- | --- |
| value | int | The mathematical element. |
| index | [MathNaryOperator](../../mathnaryoperator) | The zero-based index of the item |


---


## set_Item(int index, [MathRadical](../../mathradical) value)  function

 Gets or sets IMathElement at the specified index.
 

### Parameters

| Name | Type | Description |
| --- | --- | --- |
| value | int | The mathematical element. |
| index | [MathRadical](../../mathradical) | The zero-based index of the item |


---


## set_Item(int index, [MathBorderBox](../../mathborderbox) value)  function

 Gets or sets IMathElement at the specified index.
 

### Parameters

| Name | Type | Description |
| --- | --- | --- |
| value | int | The mathematical element. |
| index | [MathBorderBox](../../mathborderbox) | The zero-based index of the item |


---


