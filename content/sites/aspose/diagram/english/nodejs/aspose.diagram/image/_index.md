---
title: "Image Class"
linktitle: "Image"
articleTitle: "Image"
second_title: "Aspose.Diagram for Node.js via Java"
description: "Contains the gamma, brightness, contrast, blur, sharpen, denoise, and transparency values for a bitmap."
type: docs
weight: 1180
url: /nodejs/aspose.diagram/image/
---

## Image class

Contains the gamma, brightness, contrast, blur, sharpen, denoise, and transparency values for a bitmap.

## Methods

| Name | Description |
| --- | --- |
| [getBlur()](./getblur/) | Blurs or softens a bitmap image. The Blur element contains a value between 0 and 1; the default value is 0. |
| [getBrightness()](./getbrightness/) | Adjusts the brightness of a bitmap image. The Brightness element contains a value between 0 and 1; the default value is  |
| [getContrast()](./getcontrast/) | Specifies the contrast of a bitmap image. The Contrast element contains a value between 0 and 1. |
| [getDel()](./getdel/) | A flag indicating whether the element has been deleted locally. A value of 1 indicates that the element was deleted loca |
| [getDenoise()](./getdenoise/) | Removes noise (pixels that have randomly distributed color levels) from a bitmap image. |
| [getGamma()](./getgamma/) | Adjusts or corrects the intensity of an image for a particular output device, such as a monitor or scanner. The default  |
| [getSharpen()](./getsharpen/) | Specifies the amount to sharpen a bitmap image. Sharpening an image focuses it by increasing the contrast of adjacent pi |
| [getTransparency()](./gettransparency/) | Specifies the transparency level for a layer color, from 0 (opaque) to 1 (completely transparent). |
| [setDel()](./setdel/) | A flag indicating whether the element has been deleted locally. A value of 1 indicates that the element was deleted loca |
