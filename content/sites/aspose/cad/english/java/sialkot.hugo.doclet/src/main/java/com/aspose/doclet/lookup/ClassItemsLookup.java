...
import com.aspose.doclet.model.ExceptionItem;
import com.aspose.doclet.model.MethodParameter;
import com.aspose.doclet.model.Return;

import com.aspose.doclet.utils.CommentHelper;
import com.aspose.doclet.utils.StringUtil;
import com.aspose.doclet.utils.Utils;
...
public class ClassItemsLookup extends BaseLookup<Element> {
    ...
    protected ExtendedMetadataFileItem buildMetadataFileItem(Element element) {
        ...
        ExtendedMetadataFileItem result = new ExtendedMetadataFileItem(uid) {{
            ...
            setShortSummary(determineShortComment(element));
        }};
    ...
    List<MethodParameter> extractParameters(ExecutableElement element) {
        return element.getParameters().stream().map(o -> {
            ...
            if(o.asType().getKind() == TypeKind.ARRAY)
            {
                isArray = true;
            }
            Element e = getElementByType(isArray?paramType.replace("[]",""):paramType);
            String packageName = "";
            String typeName = "";
            if(e != null) {
                typeName = e.getSimpleName().toString();
                packageName = determinePackageName(e);
            }
            return new MethodParameter(paramName, paramType, extractParameterDescription(element, paramName), packageName,typeName, isArray);
        }).collect(Collectors.toList());
    }

    String extractParameterDescription(ExecutableElement method, String paramName) {
        return StringUtil.RemoveClassPrefixFromLinks(getDocCommentTree(method).map(docTree -> docTree.getBlockTags().stream()
        ...
        ).orElse(null), method.getEnclosingElement().getSimpleName().toString());
    }
    ...
    Return extractReturn(ExecutableElement methodElement) {
        ...
        String returnType = String.valueOf(methodElement.getReturnType());
        Element element = environment.getTypeUtils().asElement(methodElement.getReturnType());
        if(element !=null && getElementByName(element.getSimpleName().toString()) !=null)
        {
            returnType = element.getSimpleName().toString();
            returnPackage = determinePackageName(element);
        }
        return new Return(returnType, extractReturnDescription(methodElement), returnPackage);
    }

    String extractReturnDescription(ExecutableElement methodElement) {
        return StringUtil.RemoveClassPrefixFromLinks(getDocCommentTree(methodElement).map(docTree -> docTree.getBlockTags().stream()
                .filter(o -> o.getKind() == Kind.RETURN)
                .map(o -> (ReturnTree) o)
                .map(o -> replaceLinksAndCodes(o.getDescription()))
                .findFirst().orElse(null)
        ).orElse(null), methodElement.getEnclosingElement().getSimpleName().toString());
    }
