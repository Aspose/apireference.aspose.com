...
import com.aspose.doclet.model.ClassInfo;
import com.aspose.doclet.model.MetadataFileItem;
import com.aspose.doclet.model.TypeParameter;
...
public class ClassLookup extends BaseLookup<TypeElement> {
    ...
    protected ExtendedMetadataFileItem buildMetadataFileItem(TypeElement classElement) {
        ...
        result.setShortSummary(determineShortComment(classElement));
    ...
    void populateContent(TypeElement classElement, String shortNameWithGenericsSupport,
        ...
        if (CollectionUtils.isNotEmpty(interfaces)) {
            ...
            for (TypeMirror interfaceInfo: interfaces
                 ) {
                TypeElement interfaceElement = (TypeElement) environment.getTypeUtils().asElement(interfaceInfo);
                container.getInterfaces().add(getClassInfoFromElement(interfaceElement));
            }
    ...
    private ClassInfo getClassInfoFromElement(TypeElement typeElement)
    {
        ClassInfo classInfo = new ClassInfo();
        classInfo.setQualifiedName(typeElement.getQualifiedName().toString());
        classInfo.setName(typeElement.getSimpleName().toString());
        if(getElementByType(classInfo.getQualifiedName()) !=null) {
            classInfo.setPackageName(determinePackageName(typeElement));
            classInfo.setExternalClass(false);
        }
        return classInfo;
    }
    List<ClassInfo> determineNestedSuperclass(TypeElement classElement, ExtendedMetadataFileItem result, List<ExtendedMetadataFileItem> inheritedMethods) {
        List<ClassInfo> nestedList = new ArrayList<>();
        ...
        if (superclass.getKind() != TypeKind.NONE) {
            ...
            nestedList.add(getClassInfoFromElement(superClassElement));
